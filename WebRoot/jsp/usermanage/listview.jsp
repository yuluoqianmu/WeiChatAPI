...
<html>
  <head>
    ...
    <script type="text/javascript" src="<%=request.getContextPath()%>/scripts/checkUtils.js"></script>
  </head>
...
#preview-pane .preview-container {
   ...
   <script type="text/javascript">
	...
	function changeMobile(){
		if(confirm("您确定要修改手机号吗？")){  
			var oldMobileValue=$("#mobile").val();
			//alert('old = ' + oldMobileValue)
			...
			if(!$("#mobile").readonly){
			      ...
			      if(checkOneMobile(mobileValue) == true){
			         if(oldMobileValue != mobileValue){
			            $.ajax({
			    		  type:"post",
			  			  url:"changeMobile.action",
			  			  data:{userId:userId,mobile:mobileValue},
			  			  async:true,
			  			  dataType:"json", 
			  			  success:function(data){
  ...
  function Check_FileType(str)
{
 ...
 function AutoResizeImage(maxWidth, maxHeight, objImg) {
  ...
  <body width="98%">
    ...
    <fmt:formatDate value="${user.registerTime }" var="registerTime" pattern="yyyy-MM-dd HH:mm:ss"/>
    <li><label ><font class="labelfontstyle">注册时间:</font></label><input name="registerTime" type="text" class="dfinput" readonly="readonly" value="${registerTime}"/></li>
    <li><label ><font class="labelfontstyle">第三方登录:</font></label>
       <c:if test="${user.accountSource==0}">
        <input  type="text" class="dfinput" readonly="readonly" value="用户注册"/>
           </c:if>
       <c:if test="${user.accountSource==1}">
                             <input  type="text" class="dfinput" readonly="readonly" value="微博用户"/>
            </c:if>
          <c:if test="${user.accountSource==2}">
                             <input  type="text" class="dfinput" readonly="readonly" value="微信用户"/>
            </c:if>
    ...
    <li><label ><font class="labelfontstyle">手机号:</font></label><input name="mobile" id="mobile" type="text" class="dfinput" readonly="readonly"  value="${user.mobile}"/><i><a href="javascript:void(0)" onclick="changeMobile()"><font class="fontstyle" color='#4098CA'>修改手机号</font></a></li>
    ...
    <div class="formtitle" style="width:790px;margin:0 auto;margin-top: 15px;"><span>上传头像</span></div> 
     ...
     <div class="preview-container">
       <img src="" id="previewId" class="jcrop-preview" alt="Preview" />   
        ...
        <div>
        <div><font style="color: #0D0D0D;">请上传不超过2M的照片(点击头像截图)</font></div>
