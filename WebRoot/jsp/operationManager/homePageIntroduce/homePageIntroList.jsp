...
<script type="text/javascript">
    $(function () {  
	    $("#add").click(function () {
	    	var url= "<%=basePath%>toHomePageEdit.action?introduceId=";
	         location.href = url;
	     });
    });
  ...
  function deleteArticles(){
		...
		if(confirm("确定要删除吗?")) {
			document.location="<%=request.getContextPath()%>/deleteHomePageArticle.action?introduceId=" + ids;
		}
  }
...
function getArticleDetail(introduceId){
	var url= "<%=basePath%>getHomePageArticleDetail.action?introduceId="+introduceId;
	location.href = url;
}
...
<body>
<form id="introduceForm" name="introduceForm" method="post" action="<%=request.getContextPath()%>/queryHomePageIntroduce.action">
	<input type="hidden"  name="hasOtherOnline" id="hasOtherOnline" value="<s:property  value='#request.hasOtherOnline'/>"  />
	<div class="place">
    <span>位置：</span>
    <ul class="placeul">
    <li><a href="#">首页</a></li>
    <li><a href="javascript:<%=request.getContextPath()%>/queryHomePageIntroduce.action;">运营后台</a></li>
    <li><a href="#">首页推广</a></li>
    </ul>
    </div>
	...
	<s:if test="#request.list!=null && #request.list.size()>0">
		<s:iterator value="#request.list" id="list" status="i">
	        <tr align="center" ondblclick="getArticleDetail('<s:property  value="#list.introduceId"/>');">
	        <td><input type="radio"  name="ids" value="<s:property  value='#list.introduceId'/>"  /></td>
	        <td><s:property  value="#i.getCount()"/></td>
	        <td title="<s:property  value='#list.title'/>">
	        	<a href="javascript:getArticleDetail('<s:property  value="#list.introduceId"/>');">
	        ...
	        <td><img src='<s:property  value="#list.imgUrl"/>' title='上线' height='70' /></td>
	        <td><s:property  value="#list.onLineTime"/></td>
	        <td><s:property  value="#list.offLineTime"/></td>
	        ...
	        <td>
		        <s:if test="#list.status == 0">
					<a href="javascript:online('<s:property value="#list.introduceId"/>');"><img src='<%=basePath%>images/online.png' title='上线' /></a> 
				</s:if>
				<s:if test="#list.status == 1">
					<a href="javascript:offline('<s:property value="#list.introduceId"/>');"><img  src="<%=basePath%>images/offline.png" title='下线' /></a>
				</s:if>
			</td>	
	        </tr>
     	</s:iterator>
    ...
    <table width="98%" class="splitPage">
		<tr>
			<td class="zi_3F6293" align="right">
				<page:page name="pageController" frmName="introduceForm" title="记录" unit="条记录" actionName="queryHomePageIntroduce.action"/>
			</td>
		</tr>
	</table>
