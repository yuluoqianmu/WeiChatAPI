...
<%--     function addComment(){
    ...
    function deleteCommentById(commentID){
     if(confirm("确定要删除吗?"))
			{
			     var galleryId=$("#galleryid").val();
				document.location="<%=request.getContextPath()%>/galleryManAction_deleteCommentById.action?commentid="+commentID+"&galaryId="+galleryId;
    ...
    function deletePhotoFromDetail(photoid){
    	var galaryId = "<s:property value='#request.gallery.galaryId'/>";
    	var url = "galleryManAction_deletePhotoById.action?photoid="+ photoid +"&galaryId=" + galaryId;
    	location.href = url;
    }
...
flag=true;
  ...
  <c:if test="${JSP ne 'YES' }">
   ...
   <div id="label1" class="formtitle"><span><a href="javascript:void(0)" >单个作品集</a></span></div>
     ...
     <div id="firstDIV">
      <div id="subjectDiv">
       <table id="textTable" class="tablelist">
        <input type="hidden" name="JSP" id="JSP" value="${JSP}"/> 
       <tbody>
        <tr>
        <td>作品主题：</td>
        <td>
    ...
    <img onload="javascript:DrawImage(this)" width="150" src="${item.photoSrc}" title="点击看大图"/>
    </a>
    </span>
    ...
    <p></p>
      ...
      <s:property value='#request.gallery.lpService.serviceName'/></span></div>
       ...
       <s:if test="#request.extend!=null">
        <table style="margin-left: 2%;" id="commentTable" class="tablelist">
       	<thead>
    	<tr>
        <th>价格</th>
        <th>拍摄时间</th>
     ...
     <div id="label1" class="formtitle"><span><a href="javascript:void(0)" >评论列表</a></span></div>
       ...
       <tbody>
        ...
        <td>
          <c:if test="${JSP ne 'YES' }">
        <span id="addComment"><img src="images/t01.png" title="添加评论"/></span>
         </c:if>
        </td>
        <td>
         <c:if test="${JSP ne 'YES' }">
        <span><a id="updateNumber" href="javascript:updateNumber();"><img src="images/t02.png" style="height: 24px" title="修改浏览数量、赞数量"/></a></span>
         </c:if></td>
        </tr>
        </tbody>
