      ...
      $("#submitImage").bind("click",function(){
    	  /**获取相关参数*/
    	   ...
    	   if(gallerySubject==null||gallerySubject==""){
    	      alert("亲~您没有填写作品主题哦~→_→→_→→_→");
    	      return false;
    	    }
    	   //服务描述
    	   var galleryDesc=$("#galaryDesc").val();
/*     	    if(galleryDesc==null||galleryDesc==""){
    	      alert("亲~作品描述也是要有的哦~→_→→_→→_→");
    	       return false;
    	    } */
    	   //用户账号
    	   var  account=$("#account").val();
    	    if(account==null||account==""){
    	      alert("亲~没有填写用户账号~→_→→_→→_→");
    	       return false;
    	    }
    	    //alert(accountflag);
    	    if(!accountflag){
    	     alert("亲~都告诉你账号不正确了，咋还提交呢？→_→→_→→_→");
    	       return false;
  			  ...
  			  success:function(data){
  				...
  				if(data.result==0){
  					...
  					setTimeout(function(){
  						alert("上传成功");
  						$("#close").trigger("click");
  						if(document.getElementById("service_galary").value!=""){
  							var cameraServiceJSP=document.getElementById("cameraServiceJSP").value;
  							location.href="<%=request.getContextPath()%>/serviceAction_queryById.action?serviceId="+serviceId+"&JSP="+cameraServiceJSP;
  						}
  							},300);
  					window.location.href="galleryManAction_queryall.action";		
    		  ...
    		  type:"post",
  			  ...
  			  success:function(data){
  			   ...
  			   if(data!=null && data.length>0){
  			    ...
  			    $ul=$("<ul id='serviceStyle' style='margin-left:41px;'></ul> "); 
  			   	$div.append($ul);
...
<body>
<input type="hidden" id="service_galary" value="${service.serviceId }"/>
<input type="hidden" id="cameraServiceJSP" value="${cameraServiceJSP }"/>
...
<s:if test="#request.service==null">
  <div id="queryservice">
   <div id="userId">
    <ul class="forminfo">
    <li><label class="regfont">用户账号</label><input type="text" id="account" name="userId"  class="myinput"/>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
       <input type="button" id="userAccount" value="查询用户服务" class="btn" style="padding:0;height:34px;" onclick="queryByAccount()"/>
    </li>  
    </ul> 
   </div>
  
  <div id="selectDiv">
   <span style="margin-left:42px;"><input id="private" type='radio' name='service' value="private" checked="checked"/>&nbsp;&nbsp;<label for="private">私人订制</label></span>
   </br>
    </div>
    </div>
    </s:if>
    <s:else>
     <div id="queryservice">
        ...
        <ul id='serviceStyle' class='forminfo' >
        <li><input type="text" id="account" name="userId" value="${service.lpUser.userName }" class="myinput" readonly="readonly"/></li>
         </br> </br>
         <li><input type='radio' name='service' value="${service.serviceId }" checked="checked"/>${service.serviceName }</li>
        </ul>
      </div>
    </s:else>
   <div id="label4"  class="formtitle"><span>主 &nbsp;&nbsp;题</span></div>
...
<input  type="button" class="btn" id="submitImage" style="margin-top:5px;padding:0" value="上传并保存所有数据" />
 </div> 
