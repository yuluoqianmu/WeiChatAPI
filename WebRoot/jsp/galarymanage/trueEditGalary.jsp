  ...
  <script type="text/javascript">
   var jcrop_api;
	...
	function backToDetail(galleryID){
		location.href="galleryManAction_toEdit.action?galaryId="+galleryID;
	}
...
/*      //从作品集详情中删除一张照片
   function deletePhotoFromDetail(photoid){
    	 ...
    	 url= "galleryManAction_deletePhotoById.action?photoid="+ photoid +"&galaryId=" + galaryId;
    	location.href = url;
    } */
   ...
   <div id="label1" class="formtitle"><span><a href="javascript:void(0)" >作品集照片编辑 ( 编辑完成，请点击最底下的“提交”按钮，保存数据 )</a></span></div>
     ...
     <div id="firstDIV">
       <table id="textTable" class="tablelist" style="width:60%;height: 50%">
       	...
       	<br/>
       	<li style="margin-left: 5%"><label>发布时间：</label><s:date name="#request.gallery.creatTime" format="yyyy-MM-dd HH:mm:ss"/></li>
       	<br/>
     ...
     <div id="twoDIV">
		...
		<c:if test="${listphoto!=null&& fn:length(listphoto)>0}">
	       <c:forEach items="${listphoto}" var="item">
		    <li id="galaryCover" style="width: 200px;height: 150px" >
		     <div onclick="selectImg('${item.photoId}');">
		    <span style="text-align:center;"><img id="${item.photoId}" width="150"  <c:if test="${item.galaryConver == true}">class="thisclass"</c:if> onload="javascript:DrawImage(this)" src="${item.photoSrc}" onclick="selectCover(this)"/></span>
     ...
     <li style="margin-left: 5%"><input name="submit"  type="submit" class="btn" value="提交" class="dfinput" />
