...
import com.laipai.config.ApiConfig;
...
//		tool.openWriter(true);
	...
	public void writeLog(String log, String tag){
		...
		try {
			byte[] logBytes = log.getBytes(config.getDefaultEncode());
			this.writeLog(logBytes, 0, logBytes.length, tag);
		} catch (UnsupportedEncodingException e) {
			logger.error("unknown encode",e);
		}
	}
	/**
	 * 写日日志
